...
#>>>>>>>>>>>>>>>>>>>>>>>>>>functions>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
def click_button(item):                                           # 'click_button' function continuously updates the input field whenever you enter a number
    global expression
...
def equal_button():
    result = ""
    try:
        result = eval(inputText.get())                             # 'eval' function evaluates the string expression directly
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Frame>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
expression = ""
inputText = StringVar()                     # 'StringVar()' is used to get the instance of the input field
